...
use sui_types::transaction::{
    Argument, CheckedInputObjects, GasData, InputObjects, ObjectReadResult, TransactionKind,
};
...
impl Guest for Init {
    fn init() -> () {
        ...
        let executor = sui_execution::executor(&protocol_config, false)
            .expect("Failed to init sui execution context");

        let mut store = InMemoryStorage::new(Vec::new());

        let move_std_package_modules_bytes = MOVE_STD_MODULES_BYTES.map(|bytes| {
            base64::engine::general_purpose::STANDARD
                .decode(bytes)
                .unwrap()
        });
        let move_std_package_modules = move_std_package_modules_bytes.map(|bytes| {
            CompiledModule::deserialize_with_config(&bytes, &protocol_config.binary_config(None))
                .unwrap()
        });
        ...
        let sui_package_modules_bytes = SUI_FRAMEWORK_MODULES_BYTES.map(|bytes| {
            base64::engine::general_purpose::STANDARD
                .decode(bytes)
                .unwrap()
        });
        let sui_package_modules = sui_package_modules_bytes.map(|bytes| {
            CompiledModule::deserialize_with_config(&bytes, &protocol_config.binary_config(None))
                .unwrap()
        });
