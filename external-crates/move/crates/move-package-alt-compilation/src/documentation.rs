...
pub fn build_docs(
    ...
    let package_root_doc_template = package_root
        .join(SourcePackageLayout::DocTemplates.path())?;
    let root_doc_templates = find_filenames_vfs(
        &[package_root_doc_template.clone()],
        |path| path.extension().is_some_and(|ext| ext.as_str() == "md"),
    )
