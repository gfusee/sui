...
mod tests {
    ...
    fn vpath(path: impl AsRef<std::path::Path>) -> VirtualPath {
    ...
    async fn convert_no_envs() {
        ...
        let pubs = load_legacy_lockfile(&vpath(&lockfile)).unwrap();
    ...
    async fn convert_with_envs() {
        ...
        let pubs: ParsedPublishedFile<Vanilla> =
            load_legacy_lockfile(&vpath(&lockfile)).unwrap().unwrap().into();
    ...
    async fn convert_malformed() {
        ...
        let pubs = load_legacy_lockfile(&vpath(&lockfile));
