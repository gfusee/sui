...
impl<L> Server<L> {
    ...
    #[cfg(feature = "tls")]
    pub fn tls_config(self, tls_config: tokio_rustls::rustls::ServerConfig) -> Result<Self, Error> {
        let tls_acceptor = TlsAcceptor::new(tls_config).map_err(Error::from_source)?;
        Ok(Server {
            tls: Some(tls_acceptor),
            ..self
        })
    }
